...
- Before proceeding to a new implementation phase, ensure the full unit-test and integration-test suites pass for the current codebase state.
...
| Solution/project/active file/open documents/selection context | P01, P05 | P05.01–P05.07 implemented (metadata collectors, bridge-backed active-document text, selection kind inference, bridge-backed semantic target capture, scoped compiler diagnostics, truthful test-failure/Git-status unavailability, and privacy/exclusion/redaction, all with tested fallback) |
| Build errors, compiler warnings and test failures | P01, P05, P06 | P05 implemented scoped active-document compiler diagnostics and a truthful test-failure unavailable snapshot (Test Window remains a private-API gap); P06 build/test-run integration remains |
| Git branch, status and recent commits | P05 | P05.05 implemented: bounded, read-only `git` CLI collection with a 5-second timeout and honest unavailable fallback |
| Context automatically included in prompts, with preview/privacy controls | P05, P07 | P05.06 implemented exclusions/redaction/budgets/diagnostics for all context sources including compiler diagnostics; P07 prompt-inclusion UI remains |
...
Copilot Chat is the preferred user experience whenever a supported Visual Studio/Copilot API can open or hand off an eligible workflow truthfully. The extension must treat direct Copilot handoff as a capability-checked host integration, not as permission to automate unsupported response retrieval, edit application or hidden session control. When supported handoff is unavailable, the extension falls back to explicit prompt preview/edit/copy and manual result import without pretending that Copilot steps executed automatically.
...
- Prefer handing eligible upstream workflow steps into GitHub Copilot Chat through supported APIs so the chat surface is primary where proven. Keep workflow state, approvals, evidence, privacy, persistence and recovery in extension-owned contracts rather than assuming the chat session becomes the system of record.
...
- [x] P03.09 Implement versioned local caching and rollback with active-run protection and explicit retention controls. `TheKameleon.Superpowers.Core` now includes minimal cache-state and rollback result contracts, and `TheKameleon.Superpowers.Skills` now includes a `CatalogCacheManager` that imports validated activated downloads into versioned cache directories, tracks the active release with a simple filesystem pointer, supports rollback to any cached version, prunes older unpinned releases according to an explicit retention count, and preserves cached releases referenced by active-run pins. `CatalogCacheManagerTests` cover cache import, rollback, retention pruning, and active-run protection for pinned cached releases.
- [x] P03.10 Test catalog completeness, prereleases, offline selection, corrupt/malicious/cancelled downloads, incompatible versions, rollback and upgrade selection preservation. Added discovery coverage for prerelease-inclusive completeness and non-success remote responses; added download coverage for malicious archive traversal, cancelled downloads, and incompatible staged catalogs; added cache coverage for missing-release rollback; and added reload coverage to preserve the selected skill across catalog upgrades when it remains available. `ApprovedReleaseDownloadServiceTests` now includes async client helper support for cancellation scenarios. Validated with a clean `TheKameleon.Superpowers.Tests` build and focused passing test runs for the new P03.10 methods.
...
- [x] P04.01 Implement explicit adapter run/task states and evidence gates, separate from upstream instructional prose. `TheKameleon.Superpowers.Core` now includes a portable run-tracking contract family under `Contracts/Runs` for schema-versioned adapter runs, task records, evidence records, evidence requirements, and explicit run/task/evidence state enums. Evidence gating is evaluated explicitly through `AdapterEvidenceGateEvaluator`, so completed tasks require observed or allowed imported evidence and cannot silently pass with missing, stale, or blocked evidence. `TheKameleon.Superpowers.Tests/AdapterRunStateTests.cs` covers valid construction, invalid state/schema rejection, evidence-gate satisfaction/failure, run-level violation reporting, and JSON round-tripping/forward compatibility. Validated with a clean `TheKameleon.Superpowers.Tests` build and a passing full assembly test run.
- [x] P04.02 Implement upstream skill selection/composition, accepted plan task IDs and capability-aware handoff/waiting without fabricating agent actions. `TheKameleon.Superpowers.Core` now carries portable plan-composition metadata (`PlanEntryPointMetadata`, `PlanCompositionStep`) plus capability/handoff/accepted-task contracts (`CapabilitySnapshot`, `HandoffDecision`, `AcceptedPlanTask`, `SkillCompositionRecord`, `CompositionResolutionResult`) and extends `AdapterRunRecord`/`LoadedCatalogRelease` to persist composition and capability state explicitly. `BundledCatalogLoader` now parses and validates per-release `plan-metadata.json`, and `TheKameleon.Superpowers.Skills/Composition/SkillCompositionCoordinator.cs` resolves the Plan entry point from metadata, maps the referenced upstream skills, records accepted plan task IDs, and returns capability-aware waiting states or manual preview/copy fallback instead of pretending unsupported Copilot work executed automatically. `SkillCompositionCoordinatorTests` and extended `BundledCatalogLoaderTests` cover successful composition, missing skills, duplicate accepted task IDs, manual-only fallback, and plan-metadata loading. Validated with a clean `TheKameleon.Superpowers.Tests` build and focused passing test runs for the new P04.02 methods.
- [x] P04.03 Implement pause/cancel/retry semantics and protection against duplicate side effects. `TheKameleon.Superpowers.Core` now includes portable lifecycle contracts for run controls, action attempts, side-effect kinds, retry dispositions, and lifecycle results, and extends `AdapterRunRecord`/`AdapterTaskRecord` to persist control requests and per-task attempts explicitly. `TheKameleon.Superpowers.Skills/Composition/RunLifecycleCoordinator.cs` now applies pause/cancel/retry transitions, records control events, cancels non-terminal tasks during run cancellation, and blocks automatic retry when a prior successful attempt already produced a non-idempotent side effect. `RunLifecycleCoordinatorTests` covers valid pause/cancel flows, allowed failed-task retries, duplicate non-idempotent retry blocking, idempotent retry allowance, and invalid transition rejection. Validated with a clean `TheKameleon.Superpowers.Tests` build and focused passing test runs for the new P04.03 methods.
- [x] P04.04 Implement local versioned persistence with atomic writes and recovery. `TheKameleon.Superpowers.Core` now includes explicit workflow persistence contracts (`WorkflowPersistenceEnvelope`, save/load results, history snapshots/queries/export options, and resume request/validation DTOs), and `TheKameleon.Superpowers.Skills/Workflow/WorkflowStore.cs` provides local JSON persistence with atomic writes (`.tmp` + replace/move), backup files, recovery from unreadable or corrupt primary files, and rejection of unsupported future persistence schema versions. The persistence envelope remains schema-versioned and stores run state, selected skill, release commit, policy fingerprint, and optional captured context without introducing host-specific storage concerns. `WorkflowStoreTests` now covers round-trip persistence, backup recovery for unreadable and corrupt primary files, future-schema rejection, and retained history query/export/delete behavior. Validated with a clean `TheKameleon.Superpowers.Tests` build and focused passing workflow-store tests.
- [x] P04.05 Implement safe resume with snapshot/skill/policy validation. `TheKameleon.Superpowers.Core` now includes `WorkflowResumeResult` plus a `Resume` control action, and `TheKameleon.Superpowers.Skills/Workflow/WorkflowResumeService.cs` restores persisted workflow state only when workspace, release, selected-skill, context snapshot, and policy expectations still match. `WorkflowResumeValidator` now rejects mismatched workspace/release/skill state, changed active-document or snapshot timing, and missing-or-changed policy fingerprints when current policy constraints are provided; resumable active runs are normalized to a paused state and persist an explicit resume control instead of silently continuing execution. `WorkflowStoreTests` now covers successful resume, validation failures, load-failure propagation, and failed-run resume behavior, with focused passing test validation completed during P04.05 work.
- [x] P04.06 Implement metadata history query/export/delete and opt-in sensitive-content retention. `WorkflowHistoryEntry` now records whether sensitive content existed separately from any retained payload, `SuperpowersSettings` adds `RetainSensitiveHistoryContent` with metadata-only default retention, and `WorkflowStore.AppendHistory` now applies retention-window pruning plus content stripping unless sensitive history retention is explicitly enabled. `WorkflowHistoryService` gained a settings-aware append path and now exports redacted history by default unless `WorkflowHistoryExportOptions.IncludeSensitiveContent` is requested. `WorkflowStoreTests` and `SuperpowersSettingsTests` now cover metadata-only default storage, opt-in sensitive-content retention, export redaction, retention-window pruning, and settings JSON round-tripping/defaults. Validated with a clean solution build and a user-confirmed green run of 196 tests in Visual Studio.
- [x] P04.07 Add behavioral state-machine and persistence tests. `RunLifecycleCoordinatorTests` now covers pausing from `WaitingForEvidence`, rejecting cancellation from terminal run states, and rejecting retries for missing tasks in addition to the earlier pause/cancel/retry and duplicate-side-effect coverage. `WorkflowStoreTests` now adds persistence error-path coverage for blocked workspace/export targets so save, history append, delete, and export failures return diagnostics instead of throwing; this validation exposed and drove a `WorkflowStore` fix that moved workspace-directory creation into guarded persistence paths for save and append operations. Existing workflow-store coverage continues to validate corrupt/future-version state recovery and reopened-solution resume behavior. Validated with a full Visual Studio test run: 203 tests passed, 0 failed.
...
- [x] P05.01 Implement solution/project/open-document metadata collectors and active-document capture using supported host adapters. Active-document text may use the approved minimal in-process bridge only after the approved named-pipe (`System.IO.Pipes`) transport (server in `TheKameleon.Superpowers.InProcess`, client implementing `IBridgeClient` in `TheKameleon.Superpowers.Vsix/Bridge`) is proven per the checklist in `docs/superpowers/specs/p01-probe-design.md`; otherwise fall back to truthful
- [x] P05.02 Implement selection and semantic target snapshots, including unsaved document versions. `VisualStudioContextCollector.CaptureSelectionAsync` now infers a `Kind` (File/Folder) via the new pure `SelectionKindResolver` helper (`TheKameleon.Superpowers.Skills/Context`). Semantic-target capture is wired end-to-end through the approved in-process bridge: a new `BridgeOperation.GetSemanticTarget` request carries the active document's file path, captured text (including unsaved edits), and caret offset (obtained from the public out-of-process `ITextViewSnapshot.Selection.Extent.Start.Offset` API, which required no in-process caret capture); `BridgePipeServer` dispatches it to `BridgeHost.GetSemanticTargetAsync`, which delegates to the already-implemented `RoslynCapabilityProbe`/`SemanticTargetResolver`. `PipeBridgeClient`/`UnavailableBridgeClient` implement the new `IBridgeClient.GetSemanticTargetAsync`, and a new pure `ActiveDocumentBridgeResolver.EvaluateSemanticTargetCapability` helper decides whether to attempt capture based on the bridge's reported `BridgeCapability.SemanticTarget` availability. Covered by `ActiveDocumentBridgeResolverTests`, `SelectionKindResolverTests`, and a real-pipe `BridgePipeTransportIntegrationTests.ClientReceivesSemanticTargetFromRealPipeServer` test. Full suite (178 tests) passes.
- [x] P05.03 Implement scoped build-error and compiler-warning collection. The bridge protocol gained `BridgeOperation.GetCompilerDiagnostics` plus a `DocumentCompilerDiagnosticsInfo` DTO carrying `TotalCount`/`Diagnostics`; `BridgeHost.GetCompilerDiagnosticsAsync` delegates to the already-implemented `RoslynCapabilityProbe.GetDocumentCompilerDiagnosticsAsync`, and `BridgePipeServer` dispatches the new operation. `IBridgeClient`/`PipeBridgeClient`/`UnavailableBridgeClient` implement the new method, a new `ActiveDocumentBridgeResolver.EvaluateCompilerDiagnosticsCapability` helper (SPCTX714) mirrors the semantic-target capability-gating pattern, and `VisualStudioContextCollector.CaptureCompilerDiagnosticsAsync` captures diagnostics scoped to the active document's captured text (SPCTX715\u2013718) using the existing `BridgeContextMapper.MapCompilerDiagnostics`.
- [x] P05.04 Implement test-failure/run-summary collection with explicit unavailable/stale results. Since the Test Window service/result interfaces remain a private-API gap, `VisualStudioContextCollector.CaptureTestFailuresAsync` now builds the `TestFailureContextSnapshot` from the bridge's live `BridgeCapability.TestExplorer` capability detail (via `BridgeCapabilityCatalog`) instead of a hardcoded placeholder string, so the reported unavailable reason stays truthful and traceable to the actual capability probe result.
- [x] P05.05 Implement bounded Git branch/status/recent-commit collection without network or repository mutation. A new pure `TheKameleon.Superpowers.Skills/Context/GitStatusParser.cs` parses `git status --porcelain=v1 --branch` branch headers, summarizes changed-file counts without exposing diff content, and bounds/trims recent-commit lines; a new `TheKameleon.Superpowers.Vsix/Context/GitStatusCollector.cs` invokes the public `git` CLI read-only (`status --porcelain=v1 --branch`, `log -n 5 --pretty=%h %s`) with a 5-second timeout against the solution directory, returning an honest `Unavailable` snapshot when git is missing, the workspace is not a repository, or the command times out/fails, without ever mutating the repository or touching the network.
- [x] P05.06 Implement exclusions, redaction, context budgets and visible capture diagnostics before prompt inclusion. `ContextPrivacyService` gained `TransformCompilerDiagnostics`, applying the same exclusion-rule filtering (SPCTX604) and secret-pattern redaction (SPCTX602) already used for document content to each compiler diagnostic message, and wiring it into `Apply()` so compiler diagnostics participate in the same budget/exclusion/redaction pipeline as documents, build summaries, test failures, and Git status.
- [x] P05.07 Add collector, privacy and host-availability tests. New `GitStatusParserTests` cover branch-header parsing (including detached HEAD and malformed input), status summarization, and bounded/trimmed recent-commit parsing. `ActiveDocumentBridgeResolverTests` gained compiler-diagnostics-capability coverage (available/missing/unavailable-with-detail). `ContextPrivacyServiceTests` gained a compiler-diagnostics redaction/exclusion test. `BridgePipeTransportIntegrationTests` gained a real-pipe `ClientReceivesCompilerDiagnosticsFromRealPipeServer` round-trip test. Validated with a clean full solution build (0 errors) and a full `TheKameleon.Superpowers.Tests` run: 195/195 tests passed.
...
- [x] P06.01 Implement the shared capability/permission policy used by all three modes. (`ActionPolicyEvaluator` in TheKameleon.Superpowers.Skills/Execution centralizes Guided/ApprovalRequired/Full decisions per `ActionSideEffectKind`; Core adds `ActionPolicyDecision(Kind)`.)
- [x] P06.02 Implement exact-action approval, rejection, revocation and trust prompts. (`ApprovalRecord`/`ApprovalOutcome` in Core; `ApprovalService` in Skills implements Request/Approve/Reject/Revoke with strict state-transition guards.)
- [x] P06.03 Implement supported build/test runners with scoped evidence, timeouts and cancellation. (`ScopedProcessRunner`, `ScopedBuildRunner`, `ScopedTestRunner` in TheKameleon.Superpowers.Vsix/Execution run `dotnet build`/`dotnet test` with bounded output, timeout, and cancellation, returning honest `AdapterEvidenceRecord`s.)
- [x] P06.04 Implement scoped edit application with preimage checks, diff preview and conflict-safe recovery where APIs support it. (`PreimageValidator` + `DiffPreviewBuilder` in Skills; `ScopedEditApplier` in Vsix re-validates immediately before write to narrow the conflict window and blocks on mismatch instead of overwriting.)
- [x] P06.05 Implement explicitly allowlisted custom-command execution with safe arguments and bounded output. (`CustomCommandAllowlistEntry` in Core settings; `CommandAllowlistValidator` in Skills; `AllowlistedCommandRunner` in Vsix re-validates immediately before execution via `ScopedProcessRunner`.)
- [x] P06.06 Implement the supported Copilot bridge only for capabilities proven in P01. Add a dedicated VSIX-layer `ICopilotBridge` path that capability-checks supported handoff features at runtime, routes eligible requests from product workflows into GitHub Copilot Chat when the host exposes an approved API, and reports truthful partial capability rather than assuming prompt submission implies response/edit/session automation. (`ICopilotBridge` + `UnsupportedCopilotBridge` in Vsix/Bridge truthfully report no supported prompt/response/edit/session API today, per the P01 capability findings; `CopilotHandoffCoordinator` in Skills turns independently-reported capabilities into a routing decision without inferring one capability from another.)
- [x] P06.07 Implement explicit preview/copy/manual-result handoff when bridge capabilities are absent. When direct Copilot handoff is unsupported, unavailable or declined, preserve the same workflow state and prompt payload in an editable preview/copy experience, record that the run is awaiting external action, and allow manual result import without claiming unsupported automation executed. (`PreviewHandoffRecord` in Core; `PreviewHandoffService` in Skills implements Create/ImportResult/Discard transitions guarded by `PreviewHandoffState`.)
- [x] P06.08 Add policy, execution and fallback tests across Guided, Approval-required and Full modes. (New TheKameleon.Superpowers.Tests coverage: `ActionPolicyEvaluatorTests`, `ApprovalServiceTests`, `CommandAllowlistValidatorTests`, `PreimageValidatorTests`, `CopilotHandoffCoordinatorTests`, `PreviewHandoffServiceTests`, `UnsupportedCopilotBridgeTests`. Full solution build succeeded and full test suite passed (238/238).)
...
- [x] P07.01 Add reusable Remote UI/view-model components for skill selection and effective execution mode. `SuperpowersWorkflowViewModel` (VSIX) now exposes bindable release/skill/execution-mode selection backed by `AsyncCommand`/`IAsyncCommand` and `ObservableList<string>`, replacing the probe-only view model as the tool window's `DataContext`.
- [x] P07.02 Add context summary, exclusions/redaction preview and capability diagnostics. Start/Next now call the existing `VisualStudioContextCollector` (privacy-filtered context) and feed the resulting snapshot into `PromptComposer`, which honestly reports solution/document/selection state (captured/redacted/unavailable/truncated) and context diagnostics inline in the prompt preview; a dedicated separate diagnostics panel is not yet implemented.
- [x] P07.03 Add workflow progress and start/next/pause/cancel controls with valid-state enablement, delegating to the pure `WorkflowOrchestrator` (new Skills-layer coordinator over `RunLifecycleCoordinator`). Retry/resume controls reuse existing `WorkflowResumeService`/`RunLifecycleCoordinator` logic but are not yet exposed as buttons in the tool window.
- [x] P07.04 Add generated prompt preview/edit/copy without discarding user edits: the prompt `TextBox` is bound two-way and editable, and `Copy prompt` copies the current (possibly edited) text to the clipboard via `PreviewHandoffService.Create`. Explicit UI distinguishing supported Copilot Chat handoff vs manual fallback status is not yet implemented; only the preview/copy fallback path exists today, consistent with P06's truthful-fallback bridge state.
- [x] P07.05 Add searchable execution history and evidence details with retention/export/delete controls. `SuperpowersWorkflowViewModel` now persists history via the existing `WorkflowHistoryService`/`WorkflowStore` (retention/sensitive-content settings honored), exposes a `HistorySearchText` filter feeding `WorkflowHistoryQuery`, and adds Refresh/Export/Delete commands bound to new tool-window controls.
- [x] P07.06 Wire Plan to upstream brainstorming/writing-plans with captured context, design approval, visible platform guidance and accepted plan artifacts. Start now calls the existing `SkillCompositionCoordinator.ComposePlan` against the release's Plan metadata (upstream brainstorming/writing-plans composition), surfaces composed steps and the truthful handoff/capability status in `HandoffStatusText`, and `Accept design` records an `AcceptedPlanTask` and recomposes the plan so accepted artifacts are visible and distinct from generated prompts.
- [x] P07.07 Add the end-to-end Plan test and accessible UI/host checks in both IDE versions. Extended `PlanToolWindowPackageTests` asserts the packaged Remote UI XAML wires the new history/composition commands (`RefreshHistoryCommand`/`ExportHistoryCommand`/`DeleteHistoryCommand`/`AcceptDesignCommand`) and that the new interactive controls expose `AutomationProperties.Name` for accessibility; existing `WorkflowStoreTests` continue to cover the underlying history/composition persistence. Full solution build succeeded. Host-level manual verification on both VS2022 and VS2026 remains outstanding and is tracked separately, consistent with prior host-test caveats in this plan.
- [x] P07.08 Add first-use version selection for bundled releases. The tool window loads the bundled catalog (`BundledCatalogLoader.LoadFromDirectory`) on open and lets the user pick a release tag and skill from it; cached/latest/chosen network downloads, prerelease/compatibility labels, confirmation, progress/errors and rollback for those additional sources remain future work.
...
| Native F5 bridge deployment stale-cache defect resolved | Root cause: native `DeployVsixExtensionFiles` (used by F5) copies the in-process bridge to a new hashed `Extensions` folder on every deployment but does not touch the sentinel file (`Extensions\\extensions.configurationchanged`) Visual Studio's Extension Manager uses to decide whether to rescan/rebuild its on-disk pkgdef cache. Without a touch, the next Exp launch kept resolving to the previous (now-deleted) folder and threw `System.IO.FileNotFoundException` loading `TheKameleon.Superpowers.InProcess.dll`, matching prior "F5 refresh regression" and stale-cache entries above. Separately, a second failure mode existed: the bridge is a `net472` assembly that cannot be reloaded into an already-running Exp `devenv.exe` process, so F5 while an old Exp session was still open silently redeployed without effect. Fix: `build/BridgeDebugDeployment.targets` now adds (1) `CheckNoRunningExpInstance`, invoked from `ValidateNativeBridgeDebugTarget` (`BeforeTargets="PrepareForBuild"`), which fails the Debug build immediately with an actionable message if a target Exp instance is already running; and (2) `RefreshExpExtensionCache` (`AfterTargets="DeployVsixExtensionFiles"`), which touches `$(ExtensionsPath)\\extensions.configurationchanged` using the same `ExtensionsPath` VSSDK's own `GetVsixDeploymentPath` target computes, so the next Exp launch always rescans instead of using a stale cached folder mapping. Neither change touches ordinary (non-Debug, non-VS-hosted) builds; `NativeDeploymentRequiresSafeDebugSelectionAndNeverInstallsDuringBuild` continues to assert no `Exec` tasks exist in this file. Verified directly: invoked the same `PrepareBridgeDebug` MSBuild target F5 uses with the real `DeployTargetInstanceId`, and confirmed the sentinel file's timestamp updated to match the new deployment (first attempt caught and fixed a path-concatenation bug: `$(ExtensionsPath)` has no trailing separator). Full solution build and all 59 `TheKameleon.Superpowers.IntegrationTests` (including all 14 `BridgeDebugDeploymentTests`) passed. The user then confirmed F5 works end-to-end on both VS2022 and VS2026 Exp instances without any manual `devenv /UpdateConfiguration` step. This closes the "Automatic F5 refresh remains an open debugging-workflow defect" item recorded earlier in this log for both IDE generations. |

| Bridge transport checklist items 2/3/5/6 closed with new automated tests | Following user-confirmed live `GetActiveDocumentText` round trips on both VS 2022 and VS 2026 (real file content, matching length, live bridge response not `UnavailableBridgeClient`), the remaining P01 bridge-transport-proof checklist items from `docs/superpowers/specs/p01-probe-design.md` were closed with 5 new tests added to `TheKameleon.Superpowers.Tests/BridgePipeTransportIntegrationTests.cs`, exercised against real `NamedPipeServerStream`/`NamedPipeClientStream` pairs (not mocks): `ServerReturnsStructuredVersionMismatchFailure` (item 2 - version mismatch produces a structured `BridgeFailureKind.VersionMismatch` response), `ClientCancellationDuringSendPropagatesAndDoesNotThrowUnexpectedException` and `ServerAcceptLoopStopsAfterDisposeWithoutThrowing` (item 3 - cancellation/shutdown), `ServerDispatchConvertsUnhandledExceptionToStructuredInternalErrorResponse` (item 5 - an unhandled capability-handler exception is converted to a structured `BridgeFailureKind.InternalError` response, never a raw exception/stack trace), and `ServerPipeAclGrantsOnlyCurrentUserReadWriteAccess` (item 6 - reads back the real OS pipe ACL via `GetAccessControl()` and asserts a single `ReadWrite` `Allow` rule scoped to `WindowsIdentity.GetCurrent().User`). All 8 tests in the file pass; a full solution build and the full `TheKameleon.Superpowers.Tests` suite (159 tests) pass. The only remaining checklist item is item 4 (disconnect/reconnect), which requires a manual runtime trial (close/restart the Exp instance) rather than an automated test. |

| Bridge transport checklist item 4 (disconnect/reconnect) closed with new automated tests | Two new tests were added to `TheKameleon.Superpowers.Tests/BridgePipeTransportIntegrationTests.cs`, reusing a single `PipeBridgeClient` across two independent `NamedPipeServerStream` lifetimes on the same pipe name to simulate a bridge-server restart: `ClientReconnectsToNewServerAfterPreviousConnectionCloses` (client detects the broken cached connection via the `IOException` path in `PipeBridgeClient.SendAsync`, performs its single bounded reconnect attempt, and succeeds against the new server) and `ClientFallsBackHonestlyAfterDisconnectWhenNoReplacementServerIsListening` (when no replacement server appears, the client exhausts its bounded retry and returns the same honest unavailable shape as a cold start, without throwing or hanging). All 10 tests in the file pass; a full solution build passes. This is a same-process simulation of reconnect behavior, not an end-to-end Exp-instance restart trial; the spec now records an optional real-Exp-restart trial as further polish, not a blocker. All six items of the P01 bridge-transport-proof checklist in `docs/superpowers/specs/p01-probe-design.md` now have automated-test or live-runtime evidence: item 1 (VS2022+VS2026 live GetActiveDocumentText round trips), items 2/3/5/6 (prior milestone), and item 4 (this milestone). |

| P05.01 metadata collectors and active-document bridge capture closed with a testable resolver and passing unit tests | `VisualStudioContextCollector` (in `TheKameleon.Superpowers.Vsix/Context`) already implemented solution/project/open-document metadata collection and active-document text capture routed through the proven named-pipe bridge (falling back to metadata-only capture when the bridge or its `DocumentText` capability is unavailable), but had no dedicated automated test because its top-level `CaptureAsync` depends on the sealed, host-only `VisualStudioExtensibility` type. The bridge-trust/fallback branching was extracted into a new pure, host-independent helper, `TheKameleon.Superpowers.Skills/Context/ActiveDocumentBridgeResolver.cs` (`EvaluateCapability` and `EvaluateDocument`), and `VisualStudioContextCollector` was updated to call it instead of inlining the same logic, so the real production code path is now covered by tests rather than a parallel copy. `TheKameleon.Superpowers.Tests/ActiveDocumentBridgeResolverTests.cs` adds 6 tests covering: available/missing/unavailable `BridgeCapability.DocumentText` capability results (including preserving the bridge-reported detail message), and available/partial/unavailable bridge document snapshots. All 167 tests in `TheKameleon.Superpowers.Tests` pass (161 prior + 6 new), and a full solution build succeeds. P05.01 is now marked complete in this plan. |
