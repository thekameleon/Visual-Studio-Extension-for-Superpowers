...
- [ ] P05.01 Implement solution/project/open-document metadata collectors and active-document capture using supported host adapters. Active-document text may use the approved minimal in-process bridge only after the approved named-pipe (`System.IO.Pipes`) transport (server in `TheKameleon.Superpowers.InProcess`, client implementing `IBridgeClient` in `TheKameleon.Superpowers.Vsix/Bridge`) is proven per the checklist in `docs/superpowers/specs/p01-probe-design.md`; otherwise fall back to truthful metadata-only capture.
...
- [ ] P06.06 Implement the supported Copilot bridge only for capabilities proven in P01. Add a dedicated VSIX-layer `ICopilotBridge` path that capability-checks supported handoff features at runtime, routes eligible requests from product workflows into GitHub Copilot Chat when the host exposes an approved API, and reports truthful partial capability rather than assuming prompt submission implies response/edit/session automation.
- [ ] P06.07 Implement explicit preview/copy/manual-result handoff when bridge capabilities are absent. When direct Copilot handoff is unsupported, unavailable or declined, preserve the same workflow state and prompt payload in an editable preview/copy experience, record that the run is awaiting external action, and allow manual result import without claiming unsupported automation executed.
...
- [ ] P07.03 Add workflow progress and start/next/pause/cancel/retry/resume controls with valid-state enablement. The tool window remains the extension-owned workflow/status surface even when Copilot Chat is the primary conversation UX.
- [ ] P07.04 Add generated prompt preview/edit/copy and handoff status without discarding user edits. Show whether the last request used direct Copilot Chat handoff or manual fallback, why fallback was required, and what user action/result import is still pending.
- [ ] P07.05 Add searchable execution history and evidence details with retention/export/delete controls. Preserve enough handoff metadata to explain which steps were delegated to Copilot Chat versus completed through explicit manual fallback.
- [ ] P07.06 Wire Plan to upstream brainstorming/writing-plans with captured context, design approval, visible platform guidance and accepted plan artifacts. `PlanCommand` should prefer supported Copilot Chat handoff first and use the tool window as the truthful fallback/status experience rather than as a separate replacement authoring surface.
